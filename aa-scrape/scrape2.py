...
def convertText(text):
    return text.replace(u"\u2018", "'").replace(u"\u2019", "'").replace(u'\u201C', '"').replace(u'\u201D', '"').replace(u'\u2013', '!&-&!').replace(u'\u2014', '!%-%!').strip()
...
for row in indexFile.iter_rows(min_row=1):
    ...
    for cell in row:
        ...
        if cell.column == 'A':
            ...
            soup = BeautifulSoup(cell.value.decode('utf-8'), 'html.parser')
            ...
            if categoryType == 'class challenge':
                ...
                materials_md = soup.find('div', attrs={'class', 'activity-details-all'}).find_all('div', attrs={'class', 'activity-details'})[1]
                materials = tomd.convert(str(materials_md)).split('\n\n')[1].replace('- ', '\n-  ').strip()
                ...
                instructions_header = getText('h5', 'class', 'text-instructions')
                investigation_md = getMarkdown('article', 'class', 'panel', 1, opt_entry='div', opt_attr='class', opt_selector='panel-content')
                planning_md = getMarkdown('article', 'class', 'panel', 2, opt_entry='div', opt_attr='class', opt_selector='panel-content')
                action = getMarkdown('article', 'class', 'panel', 3, opt_entry='div', opt_attr='class', opt_selector='panel-content')
                reflection = getMarkdown('article', 'class', 'panel', 4, opt_entry='div', opt_attr='class', opt_selector='panel-content')
                demonstration = getMarkdown('article', 'class', 'panel', 5, opt_entry='div', opt_attr='class', opt_selector='panel-content')
            ...
            multiSequence = cell.value
        elif cell.column == 'F':
            description = cell.value
        line_count += 1
    if categoryType == 'class challenge':
        activitiesWorkbook[u'Class Challenge'].append([name, category, objective, materials, themes_md, preparation_md, instructions_header, introduction_md, steps_md, reflection_md, unit, grade, lesson, multiSequence, description])
    elif categoryType == 'class meeting':
        activitiesWorkbook[u'Class Meeting'].append([name, category, objective, themes_md, preparation_md, instructions_header, warm_up_md, discussion_md, reflection_md, unit, grade, lesson, multiSequence, description])
    elif categoryType == 'service learning':
        activitiesWorkbook[u'Service Learning'].append([name, category, objective, materials, themes_md, preparation_md, project_description, instructions_header, investigation_md, planning_md, action, reflection, demonstration, unit, grade, lesson, multiSequence, description])
    else:
